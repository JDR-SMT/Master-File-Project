...
namespace Dictionary
{
	public partial class FormAdmin : Form
	{
		...
		private void Save(string file)
		{
			try
			{
				Trace.WriteLine("Save to file with TextWriter.");
				Stopwatch stopwatch = Stopwatch.StartNew();

				/* overwrite existing file with StreamWriter
				...
				*/
				...
				/* overwrite existing file with WriteAllLines
				File.WriteAllLines(@file, FormGeneral.MasterFile.Select(kvp => $"{kvp.Key},{kvp.Value}"));
				*/

				stopwatch.Stop();
				Trace.WriteLine($"{stopwatch.ElapsedTicks} ticks.");
			}
			catch (Exception ex)
			{
