	...
	public partial class FormGeneral : Form
	{
		...
		private void FilterByName(string key)
		{
			...
			if (!string.IsNullOrEmpty(key))
			{
				foreach (var kvp in MasterFile)
				{
					...
					if (kvp.Value.StartsWith(key, StringComparison.OrdinalIgnoreCase))
					{
						// add kvp to list box
		...
		private void FilterById(string key)
		{
			...
			if (!string.IsNullOrEmpty(key))
			{
				...
				{
					// if kvp starts with key
					if (kvp.Key.ToString().StartsWith(key, StringComparison.OrdinalIgnoreCase))
					{
						// add kvp to list box
						ListBoxFilter.Items.Add($"{kvp.Key} {kvp.Value}");
					}
		...
		{
			// if input is minimum 3 digits
			if (TextBoxId.Text.Length >= 3)
			{
